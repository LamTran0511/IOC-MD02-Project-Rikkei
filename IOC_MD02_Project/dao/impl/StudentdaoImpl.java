...
public class StudentdaoImpl implements IStudentDao {
    ...
    public boolean findStudentByEmail(String email) throws SQLException {
        List<Student> out = list(email,"id","Asc");
    ...
    public void changePasswordWithVerify(int studentId, boolean byEmail,
        ...
        String field = byEmail ? "email" : "phone"; //là chọn cột để xác thực;
        String sqlCheck = "SELECT 1 FROM student WHERE id=? AND " + field + "=? AND password=?"; //SELECT 1 chỉ cần biết “có tồn tại bản ghi đúng điều kiện hay không”, không cần lấy full dữ liệu;
